...
# Page configuration
st.set_page_config(
    page_title="🧠 Fixed Self-Learning Crisis Detector",
    page_icon="🧠",
    layout="wide",
...
class FixedLearningSystem:
    """Fixed self-learning crisis detection system"""
    ...
    def __init__(self):
        ...
        # FIXED: More balanced base weights
        self.base_weights = {
            'crisis_keyword_count': 0.3,      # Reduced from 0.8
            'help_keyword_count': -0.2,       # Reduced from -0.6
            'text_length': 0.01,              # Reduced from 0.1
            'word_count': 0.01,               # Reduced from 0.05
            'intensity_count': 0.1,           # Reduced from 0.3
            'temporal_count': 0.15,           # Reduced from 0.4
            'negation_count': -0.1            # Reduced from -0.2
        }
    ...
    def extract_features(self, text):
        ...
        # Text features
        features['text_length'] = len(text) / 100  # Normalize
        features['word_count'] = len(text.split()) / 20  # Normalize
        
        # Emotional intensity (more conservative)
        intensity_words = ['really', 'so', 'extremely', 'completely', 'totally', 'absolutely']
        features['intensity_count'] = sum(1 for word in intensity_words if word in text_lower) / 3
        
        # Temporal markers
        temporal_words = ['tonight', 'today', 'now', 'immediately', 'right now', 'this moment']
        features['temporal_count'] = sum(1 for word in temporal_words if word in text_lower) / 2
        
        # Negation (positive indicator)
        negation_words = ['not', 'never', 'no', 'can\'t', 'won\'t', 'don\'t']
        features['negation_count'] = sum(1 for word in negation_words if word in text_lower) / 3
        
        return features
    
    def predict_with_confidence(self, text):
        """Predict with confidence score - FIXED"""
        ...
        # Calculate score with better normalization
        score = 0
        for feature, value in features.items():
            if feature in combined_weights:
                score += combined_weights[feature] * value
        
        # FIXED: Better sigmoid function with bias
        probability = 1 / (1 + math.exp(-(score - 0.2)))  # Added bias to reduce false positives
        
        # Determine prediction and confidence
        prediction = 1 if probability > 0.6 else 0  # Higher threshold
    ...
    def _update_weights(self, correction):
        ...
        # Update weights with smaller increments
        for feature, value in features.items():
            if feature not in self.feedback_data['model_weights']:
                self.feedback_data['model_weights'][feature] = 0.0
            
            # Smaller weight updates for stability
            weight_change = self.learning_rate * error * value * 0.1
    ...
    def _strengthen_weights(self, confirmation):
        ...
        for feature, value in features.items():
            ...
            # Smaller strengthening for stability
            self.feedback_data['model_weights'][feature] += self.learning_rate * 0.01 * value * predicted
...
if 'learning_system' not in st.session_state:
    st.session_state.learning_system = FixedLearningSystem()
...
def main():
    ...
    <div class="hero-section">
        <h1>🧠 Fixed Self-Learning Crisis Detector</h1>
        <p>Improved AI with balanced weights and working self-learning</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        if stats['total_feedback'] > 0:
            ...
            for feature, weight in stats['current_weights'].items():
                if abs(weight) > 0.001:  # Only show significant changes
    ...
    if st.button("🧪 Test Sample Texts"):
        ...
        for text in sample_texts:
            ...
            st.text(f"{status}: '{text}' (Confidence: {conf:.1%})")
    ...
    <div style="text-align: center; color: white; padding: 2rem;">
        <h4>🧠 Fixed Self-Learning Crisis Detector</h4>
        <p>Improved AI with balanced weights and working self-learning capabilities.</p>
